...
export const ru = {
  ...
  settings: {
    ...
    dialog: {
      title: "Настройки",
      description: "Управление настройками XOne.",
    ...
    voice: {
      ...
      dictation: {
        ...
        sttLoaded: "Загружено",
        sttUnavailable:
          "Компонент не установлен на этом сервере. Выполните `xone studio update`, чтобы включить локальную диктовку.",
        sttRetry: "Повторить",
        ...
        micAccessBlocked:
          "Доступ к микрофону заблокирован. Разрешите доступ к микрофону для этой страницы XOne и повторите попытку.",
    ...
    general: {
      title: "Общие",
      description: "Глобальные настройки XOne.",
      account: "Аккаунт",
      ...
      password: "Пароль",
      passwordDescription: "Изменить пароль для этого аккаунта XOne.",
      passwordDialog: {
        ...
        setDescription:
          "Выберите пароль, с которым удалённые браузеры входят как unsloth (не менее {minLength} символов). Настольное приложение XOne продолжит входить автоматически.",
      ...
      modelAutoSwitch: {
        ...
        apiOnlyDescription:
          "Автовыгрузка при простое оставляет в памяти модель, которую вы загрузили в XOne, и освобождает только те, что загрузил запрос через API.",
      },
      previewSharing: {
      ...
      startup: {
        ...
        launchAtLogin: "Запускать XOne при входе в систему",
        launchAtLoginDescription:
          "Запускает XOne в фоновом режиме при входе в систему. Приложение остаётся в строке меню или системном трее, пока вы его не откроете.",

        closeToTray: "Закрывать в системный трей",
        closeToTrayDescription:
          "Оставлять XOne и его сервер работающими в фоне при закрытии главного окна.",
      ...
      resetPreferences: {
        ...
        confirmDescription:
          "Очищает только локальные настройки и перезагружает XOne. Чаты, доступ по API и настройки в базе данных сохраняются.",
      ...
      permissions: {
        ...
        bypassDescription:
          "Как XOne подтверждает вызовы инструментов в чате (терминал, python, веб, MCP) перед запуском. Режим «Full access» отключает подтверждения и песочницу для кода.",
      },
    },
    profile: {
      title: "Профиль",
      description: "Как ваш профиль отображается в XOne.",
      changePicture: "Изменить фото профиля",
      displayName: "Отображаемое имя",
      nickname: "Как XOne к вам обращаться?",
      ...
      stats: {
        ...
        subtitle:
          "Всё ниже считается по вашей собственной истории. Ничего не собирается и не отправляется в XOne.",
        retry: "Повторить",
        privacyNote:
          "Статистика рассчитывается по истории чатов и обучения, которая хранится в вашей установке XOne. Ничего не собирается и ничего не отправляется ни в XOne, ни третьим лицам.",
        emptyChats:
    ...
    appearance: {
      title: "Оформление",
      description: "Как XOne выглядит на этом устройстве.",
      theme: {
        title: "Тема",
      ...
      palette: {
        ...
        description: "Цвета, используемые в XOne в светлой и тёмной теме.",
      ...
      custom: {
        ...
        uiFontSize: {
          ...
          description: "Настройте базовый размер шрифта интерфейса XOne.",
        },
        codeFontSize: {
      ...
      language: {
        ...
        description: "Язык, используемый XOne.",
    ...
    resources: {
      title: "Система",
      description: "Мониторинг оборудования и хранилища этого сервера XOne.",
      ...
      llamaBackend: {
        ...
        customPath: {
          ...
          bundled: "Используется среда llama.cpp, установленная XOne.",
        ...
        unsupported: {
          ...
          localLink: "llama.cpp, это локальный каталог, который вы подключили сами, и XOne его не заменяет.",
    ...
    agents: {
      title: "Агенты",
      description:
        "Подключение кодинг-агентов, таких как Claude Code и Codex, к локальной модели с помощью xone start.",
      intro:
        "подключает Claude Code, Codex, Hermes, OpenClaw, OpenCode и других агентов к модели, которую XOne обслуживает локально, полностью офлайн. Запускается OpenAI-совместимый сервер, а файлы конфигурации вашего агента остаются нетронутыми.",
      ...
      modelNote:
        "Codex требует модель GGUF, обслуживаемую llama-server. Другие агенты могут работать и с моделями на основе transformers; уберите --model, чтобы использовать модель, уже загруженную в XOne.",
      subagent: {
        title: "Использовать локальную модель в качестве субагента",
        description:
          "Оставьте {agent} на текущей модели и делегируйте отдельные задачи этой локальной модели XOne.",
        ...
        opencodePrompt: "@xone найди причину этого падения теста",
      },
      quickstart: {
        title: "Собрать команду",
        description:
          "Запустите агента на модели, уже загруженной в XOne. Сначала загрузите модель, затем замените claude на любого поддерживаемого агента из списка ниже.",
        noneDetected:
      ...
      options: {
        ...
        description:
          "Сначала разбираются флаги XOne; всё, что не распознано, передаётся агенту без изменений.",
        model:
          "Выбирает модель. Без --model команда xone start использует модель, уже загруженную в XOne, и завершается ошибкой, если модели нет.",
        contextLength:
        ...
        persist:
          "Сохраняет между запусками хранилище агента, управляемое XOne.",
        asSubagent:
          "Оставляет родительского агента на текущей модели и регистрирует XOne как локального субагента (Claude Code, Codex и OpenCode).",
        apiKey:
          "Передаёт ваш ключ API XOne (альтернатива: переменная UNSLOTH_API_KEY).",
        reasoning:
      ...
      remote: {
        title: "Подключение к удалённому XOne Studio",
        description:
          "Чтобы подключить xone start к удалённому экземпляру XOne Studio, задайте перед запуском следующие переменные (или передайте --api-key напрямую):",
      },
      passthrough: {
        title: "Передача аргументов агенту",
        description:
          "Аргументы после флагов XOne передаются самому агенту, поэтому его собственные команды вроде resume продолжают работать:",
      },
      dryRun: {
    ...
    remoteLan: {
      ...
      description:
        "Откройте этот XOne с других устройств: через локальную сеть или временный публичный URL.",
    },
    apiKeys: {
      title: "API",
      description: "Доступ к XOne через OpenAI-совместимый API.",
      ...
      secureHttpsHint:
        "Порт, привязанный к адресу 0.0.0.0, по-прежнему доступен глобально. Для полной безопасности запустите XOne с флагом --secure, чтобы оставить доступ только по этой HTTPS-ссылке.",
    ...
    about: {
      ...
      shutDownStudio: "Выключить XOne",
      shutDownStudioDescription:
        "Останавливает сервер XOne и завершает вашу сессию.",
      shutDown: "Выключить",
      update: {
        title: "Обновить XOne",
        ...
        checkingInstall: "Проверка способа установки XOne...",
        installIntro: "Чтобы установить или обновить XOne:",
        ...
        localInstallDetected:
          "Обнаружена установка из локальной рабочей копии. Обновляйте XOne из исходной рабочей копии, чтобы не заменить эту установку пакетом из PyPI.",
        pullThenUpdate:
        ...
        restartAfterUpdate: "После обновления перезапустите XOne.",
        desktopManaged:
        ...
        desktopExternalServer:
          "Выполните `xone studio update` в терминале, из которого был запущен сервер.",
        ...
        desktopCurrentDescription:
          "XOne продолжит автоматически проверять наличие обновлений.",
        ...
        unknownInstall:
          "Не удалось определить способ установки XOne. Для установки через установщик или PyPI используйте команды выше.",
        localCheckout:
  ...
  studio: {
    ...
    training: {
      ...
      startUnconfirmed:
        "XOne не удалось подтвердить запуск обучения. Состояние проверяется в фоновом режиме.",
