...
export function preflightStaleMessage(
  ...
  if (kind === WORKING_DIRECTORY_UNAVAILABLE) {
    const cause =
      typeof navigator !== "undefined" && /Win/i.test(navigator.platform ?? "")
        ...
        : "";
    return `XOne cannot reach your user folder, so it has nowhere to run from.${cause} Reconnect and try again.`;
  }
  ...
  if (kind === PATH_SETTING_UNRESOLVABLE) {
    const which = setting ? `${setting} points` : "One of XOne's folder settings points";
